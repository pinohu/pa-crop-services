...
const log = createLogger('client-hosting');
...
export default async function handler(req, res) {
  ...
  // Rate limit: 15/min
  const blocked = await checkRateLimit(getClientIp(req), 'client-hosting', 15, '60s');
  ...
  try {
    ...
    log.error('client_hosting_error', {}, err instanceof Error ? err : new Error(String(err)));
